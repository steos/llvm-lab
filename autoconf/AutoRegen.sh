...
echo $AUTOCONF_VERSION | egrep '2\.5[0-9]' > /dev/null
if test $? -ne 0 ; then
	echo $AUTOCONF_VERSION | egrep '2\.6[0-9]' > /dev/null
	if test $? -ne 0; then
  		die "Your autoconf was not detected as being 2.5x or 2.6x"
  	else
  		AUTOCONF_VERSION="2.6x"
  	fi
else
	AUTOCONF_VERSION="2.5x"
fi
cwd=`pwd`
...
echo "Regenerating configure with autoconf $AUTOCONF_VERSION"
